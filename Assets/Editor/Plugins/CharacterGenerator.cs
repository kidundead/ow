...
public class CharacterGenerator
{
    ...
    public bool LoadAllAsset()
    {

        if (allLoaded) return true;


        bool localAllLoaded = true;
        foreach (List<CharacterElement> category in sortedElements["female"].Values)
            foreach (CharacterElement c in category)
            {
                if (!c.Load())
                {
                    localAllLoaded = false;
                }
            }

        allLoaded = localAllLoaded;
        return allLoaded;
    ...
    public GameObject Generate(GameObject root, SkinnedMeshRenderer rootSmr)
    {
        ...
        foreach (CharacterElement element in currentConfiguration.Values)
        {
            ...
            Debug.Log(smr.sharedMesh.name + "," + smr.materials[0].name);
        ...
        foreach (CharacterElement element in currentConfiguration.Values)
        {
            ...
            foreach (string bone in element.GetBoneNames())
            {
                ...
                foreach (Transform transform in transforms)
                {
                    if (transform.name != bone) continue;
                    bones.Add(transform);
                    Debug.Log("Add Bone: " + bone);
                    break;
                }
            }
            /*
            Debug.Log("Generate Hero Start...");
            SkinnedMeshRenderer rootSmr = (SkinnedMeshRenderer) root.renderer;

            foreach (Transform bone in rootSmr.bones)
            {
                foreach (Transform transform in transforms)
                {
