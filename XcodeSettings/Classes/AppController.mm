...
extern "C" UIViewController *g_ViewController = NULL;
...
namespace {
	...
	struct ProfilerBlock _unityClearPB;
...
//	[_window addSubview:view];
	...
	// Create a full-screen window
	_window = [[UIWindow alloc] initWithFrame:rect];
	UnityViewController *unityViewController =[[UnityViewController alloc] init];
	[_window addSubview:unityViewController.view];

    
	g_ViewController = unityViewController;
	...
	if (!CreateWindowSurface(unityViewController.view, GL_RGB565_OES, GL_DEPTH_COMPONENT16_OES, NO, &_surface)) {
		return false;
	}
...
- (void) Repaint
{
	...
	if (_frameId == EachNthFrame)
	{
		...
		if (_unityClearPB.avgV >= 2.0)
		{
			printf_console("cpu-waits-gpu> min: %4.1f   max: %4.1f   avg: %4.1f\n", MachToMillisecondsDelta(_unityClearPB.minV), MachToMillisecondsDelta(_unityClearPB.maxV), MachToMillisecondsDelta(_unityClearPB.avgV / EachNthFrame));
		}
		...
		printf_console("player-detail> physx: %4.1f animation: %4.1f culling %4.1f skinning: %4.1f batching: %4.1f render: %4.1f fixed-update-count: %d .. %d\n", 
					   ...
					   MachToMillisecondsDelta((int)(_unityRenderLoopPB.avgV - _batchPB.avgV - _unityCullingPB.avgV - _unityClearPB.avgV) / EachNthFrame),
#else   
					   MachToMillisecondsDelta((int)(_unityRenderLoopPB.avgV - _oglesPB.avgV - _batchPB.avgV - _unityCullingPB.avgV - _unityClearPB.avgV) / EachNthFrame),
#endif
	...
	ProfilerBlock_Update(&_unityClearPB, _unityFrameStats.clearDt, (_frameId == 0));
...
{
	bNeedToRestoreIPhoneAudioSession = true;
