...
- (id)initWithDelegate:(id)delegate
{
	...
	if (self) {
		...
		NSString *urlAddress;
		if (isiPadSize) {
            urlAddress = [NSString stringWithFormat:@"http://www.trinitigame.com/tad/%@ipad/smallbanner.html", [NSString stringWithUTF8String:currentProjectNameUTF8]];
        }
        else {
            urlAddress = [NSString stringWithFormat:@"http://www.trinitigame.com/tad/%@/smallbanner.html", [NSString stringWithUTF8String:currentProjectNameUTF8]];
        }
...
- (id)initWithProjectName:(NSString *)projectName isAutoDispare:(BOOL)moveOut// application:(UIApplication *)application
{
	...
	if (self) {
        ...
        if ([mod isEqualToString:@"iPad"])
        {
            isiPadSize = YES;
            bannerCenter = CGPointMake(64 + 32, 32 + 480);
            bannerBounds = CGRectMake(0, 0, 960, 64);
        }
...
//{
//    self.view.frame = CGRectMake(0, 0, 480, 32);
//}
- (void)rotateForUnity:(UIDeviceOrientation)orientation
{
    if (orientation == UIDeviceOrientationLandscapeLeft) {
        //NSLog(@"self.view.center : %f, %f", self.view.center.x, self.view.center.y);
        self.view.center = bannerCenter;
        //self.view.frame = bannerBounds;
        
        //CGAffineTransform transform = banner.transform;
        //transform = CGAffineTransformRotate(transform, M_PI);
        //banner.transform = transform;
        CGAffineTransform newTrans;
        newTrans.a = 1.0f;
        newTrans.d = 1.0f;
        banner.transform = newTrans;
    }
        ...
        if ([mod isEqualToString:@"iPad"])
        {
            self.view.center = CGPointMake(768 - bannerCenter.x, bannerCenter.y);
        }
        else
        {
            self.view.center = CGPointMake(320 - bannerCenter.x, bannerCenter.y);
        }
        //self.view.frame = bannerBounds;//CGRectMake(0, 0, bannerBounds.size.height, bannerBounds.size.width);
